...
class ExtraTypeRepository
{
    use GuzzleTrait;

    public function get($data = [], $header = [])
    {
        $url = config('config.route.api') . 'admin/extra-type';
    ...
    public function find($data = [], $header = [])
    {
        ...
        $url = config('config.route.api') . 'admin/extra-type/show';
        $headers = ['Authorization' => 'Bearer ' . session('token')];
    ...
    public function update($data = [], $header = [])
    {
        ...
        $url = config('config.route.api') . 'admin/extra-type/update';
        $headers = ['Authorization' => 'Bearer ' . session('token')];
    ...
    {
        $url = config('config.route.api') . 'admin/extra-type/store';
        $headers = ['Authorization' => 'Bearer ' . session('token')];
        $user = json_decode(session('user'), true);
        $dataA = ['branch_id' => 1];
        $dataA = array_merge($dataA, $data);
    ...
    {
        $url = config('config.route.api') . 'admin/extra-type/destroy';
        $headers = ['Authorization' => 'Bearer ' . session('token')];
