<div class="relative p-2">
    <input
        ...
        placeholder="{{__('index.Search Product...')}}"
        ...
        <div class="w-1/3 bg-white rounded-lg shadow searching-select-box">
            @if(!empty($products))
                <ul class="divide-y-2 divide-gray-100 p-0 mb-0">
                    <select class="form-control form-control-solid" wire:model="$parent.productId">
                        <option class="p-2 hover:bg-blue-600 hover:text-blue-200" value="null">{{__('index.select')}}</option>
                        @foreach($products as $i => $branch)
            ...
            @else
                <div class="list-item">{{__('No results!')}}</div>
            @endif
        </div>
    @endif
</div>
